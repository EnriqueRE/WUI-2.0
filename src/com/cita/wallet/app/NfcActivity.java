...
public class NfcActivity extends Activity implements
		NativeNFCTransaction.OnNativeNFCFragmentInteractionListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (extras != null) {
			...
			if (extras.getString("title").equals("Vending")) {

				getActionBar().setIcon(R.drawable.ic_vending);

			} else if (extras.getString("title").equals("Accesos")) {

				getActionBar().setIcon(R.drawable.ic_access);
		...
		if (NFCUtils.isNFCEnabled(this)) {
			...
			getFragmentManager()
					...
					.replace(R.id.container,
							NativeNFCTransaction.newInstance("nfc", ""))
					.commit();
		...
		if (NFCUtils.hasNoNFC(this)) {
			getFragmentManager().beginTransaction()
					.replace(R.id.container, new NoNFCFragment()).commit();
		}
